  ...
  function CommandPanel({ selectedUnit, onUnitSelect, volunteers = [], incidents = [], onVolunteerAction = () => {} }) {
    ...
    useEffect(() => {
      if (selectedUnit?.status) {
        const index = statusSteps.findIndex(step => step.id === selectedUnit.status.toLowerCase());
        if (index !== -1) {
    ...
    const trustScore = {
      ...
      const handlePreviousStep = () => {
        ...
        if (selectedUnit?.id) {
          const prevStatus = statusSteps[nextIndex].id
      ...
      const handleNextStep = () => {
        ...
        if (selectedUnit?.id) {
          const nextStatus = statusSteps[nextIndex].id
